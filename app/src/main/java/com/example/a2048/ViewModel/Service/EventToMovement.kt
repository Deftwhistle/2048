...
import com.example.a2048.Utils.MovementDirection
import com.example.a2048.ViewModel.Exceptions.ErrorExceptionFinallyGame
...
class EventToMovement {
    ...
    private val verifyWinBoxInBoard: (List<Int>) -> Unit = { board ->
        if (board.find { value -> value.toInt() == 2048 } != null) {
            throw ErrorExceptionFinallyGame("Congratulations you win :)")
        }
    }
    ...
    private fun verifyBoxEmptyInBoard(board: LinkedList<Int>):Boolean{
        val element = 0
        return board.find { it == element } is Int
    }
    ...
    private fun horizontalAndVerticalMovement(
        ...
        var newBoardShort: MutableList<Int> = board.toMutableList()
